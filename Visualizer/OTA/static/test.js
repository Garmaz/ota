    ...
    g.selectAll('rect').data(data)
        ...
        .on("click", function (d) {
            ...
            d3.select(this).transition()
                ...
                .attr('opacity', '.5')
                .attr('fill','#000dff')
            var curr_assi_set = new Set()
            var curr_idle_set={};
            var curr_assi = [];
            var curr_idle = [];
            curr_assi_set.add(d.agent.id)
            data.forEach(element => {
                if (
                    ...
                    curr_assi_set.add(element.agent.id)
            ...
            data.forEach(element=>{
                Id = element.agent.id
                if(!(curr_assi_set.has(Id))){
                    // console.log(Id)
                    if(element.finish_time<d.arrivetime){
                        if (curr_idle_set[Id]){
                            if(curr_idle_set[Id][0]<element.finish_time){
                                curr_idle_set[Id] = [element.finish_time,element.request.end.x,element.request.end.y,element.agent.id]
                            }
                        }else{
                                curr_idle_set[Id] = [element.finish_time,element.request.end.x,element.request.end.y,element.agent.id]
                        }
            ...
            // temp = 0
            // for (i in curr_idle_set){
            //     temp+=1
            // }
            // console.log(temp)
            // console.log(curr_assi_set)
            for (i in curr_idle_set){
                curr_idle.push(curr_idle_set[i]);
            }
            // console.log(curr_assi)
            show_map(d.agent.id, curr_assi, curr_idle, metadata)

        })
};

function show_map(id, data, idle_data, metadata) {
    const color_map = [
        "#629af5",
        "#f04f4f"
    ]
    ...
    svg.selectAll("circle")
        ...
        .on('mouseover', function (d, i) {
            ...
            div.transition()
                .duration(50)
                .style("opacity", 1);
            div.html("Agent_ID: " + d[3] + "<br/>" + "x: " + d[1] + "<br/>" + "y: " + d[2])
                .style("left", (d3.event.pageX + 10) + "px")
                .style("top", (d3.event.pageY - 15) + "px");
        })
        .on('mouseout', function (d, i) {
    ...
    idle_data.forEach(element=>{
        svg.append("path")
            .attr("class", "diamond")
            .data(element)
            .attr("d", diamondSymbol)
            .attr("fill", "#629af5")
            .attr('transform',d=>{ return "translate("+x(element[1])+","+y(element[2])+")"; })
            .on('mouseover', function (d, i, j) {
                ...
                div.transition()
                    .duration(50)
                    .style("opacity", 1);
                div.html("Agent_ID: " + element[3] + "<br/>" + "x: " + element[1] + "<br/>" + "y: " + element[2])
                    .style("left", (d3.event.pageX + 10) + "px")
                    .style("top", (d3.event.pageY - 15) + "px");
            })
            .on('mouseout', function (d, i) {
